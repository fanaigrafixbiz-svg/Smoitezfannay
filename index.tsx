...
// FIX: Renamed `monaco` to `monacoInstance` to avoid shadowing the global `monaco` namespace, resolving a circular type reference.
...
async function addCell(
  ...
  // FIX: Added a guard clause to ensure monaco editor is initialized.
  if (!monacoInstance) {
  ...
  // FIX: Using renamed `monacoInstance` variable.
  ...
  editorInstance.onDidContentSizeChange(() => {
    ...
    const lineHeight = editorInstance
      .getOptions()
      .get(monacoInstance.editor.EditorOption.lineHeight) as number;
